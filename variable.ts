Public Class CAMT106
    Private Const tipoOperacion As String = "CAMT106"
    ...
    Shared Function procesaOperacion(ByVal icn As String, datos As Dictionary(Of String, Object)) As Boolean
        ...
        write_Log("INFO|CAMT106.procesaOperacion|A procesar el paso " & lsAliasProceso)
        ...
        If lsAliasProceso = "T24_REGISTRO" Then
            ...
            If alreadyInsertedInT24.IsSuccess
                write_Log("INFO|CAMT106.procesaOperacion|La búsqueda de FT fue exitosa.")
                ...
                If String.IsNullOrWhiteSpace(alreadyInsertedInT24.Data)
                    ...
                    If Not resultT24Insert.IsSuccess
                        If Not resultT24Insert.ErrorCode = ErrorCodeEnum.NoCode
                            ...
                            Throw New Exception("Error en procesamiento de registro CAMT106 T24_REGISTRO con folio: " & icn)
                        Else 
                            Throw New Exception("Error en procesamiento de registro CAMT106 T24_REGISTRO con folio: " & icn)
                        End If
                    End If
                    
                    write_Log("INFO|CAMT106.procesaOperacion|No se encontró ningún FT, se procede a hacer el registro en T24.")
                    res = resultT24Insert
                Else 
                    write_Log("INFO|CAMT106.procesaOperacion|Se encontró un FT asociado al mensaje: " & alreadyInsertedInT24.Data)
        ...
        If lsAliasProceso = "SWIFT_REGISTRO" Then
            ...
            If Not dispatchResult.IsSuccess Then
                write_Log("ERROR|CAMT106.ProcesaOperacion|" & dispatchResult.Reason)
    ...
    Private Shared Function UpdateCommissionContext(originalReference As String, ourReference As String) _
        ...
        If String.IsNullOrWhiteSpace(ourReference)  Then
            write_Log("ERROR|CAMT106.UpdateCommissionContext|No T24 reference passed to update commission context.")
        ...
        If String.IsNullOrWhiteSpace(originalReference)  Then
            write_Log("ERROR|CAMT106.UpdateCommissionContext|No SAMH reference passed to update commission context")
            Return Result (Of String).Failed("No SAMH reference passed to update commission context.")
        End If
        Try
            write_Log("INFO|CAMT106.UpdateCommissionContext|To update the Commission Context with the T24 reference")
            ...
            write_Log("INFO|CAMT106.UpdateCommissionContext|Payload:\n" & json)
            Using _client As New HttpClient()
                Dim _content AS New StringContent(json, Encoding.UTF8, "application/json")
                write_Log(
                    "INFO|CAMT106.UpdateCommissionContext|To update Commission Context with context ref " &
                    creditConfirmation.OriginalReference)
                Dim response AS HttpResponseMessage = _client.PutAsync(ApiNewMdwUrl, _content).Result
                write_Log("INFO|CAMT106.UpdateCommissionContext|Server's answer: " & response.ReasonPhrase)
            ...
            End Using
        Catch ex As Exception
            write_Log("ERROR|CAMT106.UpdateCommissionContext|An error occurred:" & vbCrLf & ex.Message)
    ...
    Private Shared Function DispatchMT202(ref As String) As Result(Of String)
        Try
            write_Log("ERROR|CAMT106.DispatchMT202|A enviar MT202 para terminar el cobro de la comision.")
            Dim creditConfirmation As New FiToFiCommissionCreditConfirmationDto With 
                    {
                    .TheirReference = ref
                    }
            Dim json As String = JsonConvert.SerializeObject(creditConfirmation)
            write_Log("INFO|CAMT106.DispatchMT202|Payload:\n" & json)
            Using _client As New HttpClient()
                Dim _content AS New StringContent(json, Encoding.UTF8, "application/json")
                write_Log(
                    "INFO|CAMT106.DispatchMT202|To enqueue dispatching of MT202 with context ref " &
                ...
                write_Log("INFO|CAMT106.DispatchMT202|Respuesta del servidor: " & response.ReasonPhrase)
            ...
            End Using
        Catch ex As Exception
            write_Log("INFO|CAMT106.DispatchMT202|An error occurred:" & vbCrLf & ex.Message)
            Return Result (Of String).Failed(ex.Message)
        End Try
    End Function
    ...
    Private Shared Function GetFT(rfk As String) As Result(Of String)
            write_Log("INFO|CAMT106.GetFT|A buscar FT para la comision.")
            Dim query As String =
            ...
            Try
                Using reader As DbDataReader = goTOMI_Database.Execute_Query(query)
                    write_Log("INFO|CAMT106.GetFT|Se ejecuto el query.")
                    If IsNothing(reader) Then
                        Return Result (Of String).Failed(String.Empty)
                    End If

                    If Not reader.Read() Then
                        write_Log("INFO|CAMT106.GetFT|No se encontró ningún FT asociado al CAMT106.")
                ...
                End Using
            Catch ex As Exception
                write_Log("ERROR|CAMT106.GetFT|Se presenta la excepción: " & ex.Message)
                Return Result (Of String).Failed(ex.Message)
            End Try
    End Function
End Class
